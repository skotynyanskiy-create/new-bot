...
class MeanReversionEngine:
    ...
    def detect(self, candles: deque[Candle]) -> Signal:
        ...
        volume_low = vol_ma == _ZERO or candle.volume < vol_ma * Decimal('1.3')
        ...
        if (
            ...
            if (tp - entry) > (entry - sl) * Decimal('0.8'):
                ...
                if size > _ZERO:
                    logger.info(
                        "LONG mean-rev %s | entry=%.4f tp=%.4f sl=%.4f adx=%.1f rsi=%.1f",
                        candle.symbol, entry, tp, sl, adx_val, rsi_val,
                    )
                    return Signal(
        ...
        if (
            ...
            if (entry - tp) > (sl - entry) * Decimal('0.8'):
                ...
                if size > _ZERO:
                    logger.info(
                        "SHORT mean-rev %s | entry=%.4f tp=%.4f sl=%.4f adx=%.1f rsi=%.1f",
                        candle.symbol, entry, tp, sl, adx_val, rsi_val,
                    )
                    return Signal(
                        symbol=candle.symbol,
