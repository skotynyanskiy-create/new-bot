...
class TrendFollowingEngine:
    ...
    def detect(self, candles: deque[Candle]) -> Signal:
        ...
        if (
            ...
            if size > _ZERO:
                logger.info(
                    "LONG trend-follow %s | entry=%.4f tp=%.4f sl=%.4f adx=%.1f DI+/DI-=%.1f/%.1f",
                    candle.symbol, entry, tp, sl, adx_val, di_plus, di_minus,
                )
                return Signal(
        ...
        if (
            ...
            if size > _ZERO:
                logger.info(
                    "SHORT trend-follow %s | entry=%.4f tp=%.4f sl=%.4f adx=%.1f DI-/DI+=%.1f/%.1f",
                    candle.symbol, entry, tp, sl, adx_val, di_minus, di_plus,
                )
                return Signal(
                    symbol=candle.symbol,
